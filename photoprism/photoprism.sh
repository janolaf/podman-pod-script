...
podman container create --name photoprism-db --pod photoprism \
	...
	--secret photoprism_mysql_root_password,type=env,target=MYSQL_ROOT_PASSWORD \
	-e MYSQL_DATABASE=photoprism \
	-e MYSQL_USER=photoprism \
	# -e MYSQL_PASSWORD=insecure \
	--secret photoprism_mysql_password,type=env,target=MYSQL_PASSWORD \
...
podman container create --name photoprism-app --pod photoprism \
	...
	--secret photoprism_password,type=env,target=PHOTOPRISM_ADMIN_PASSWORD="insecure" \
